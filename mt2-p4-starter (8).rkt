...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |mt2-p4-starter (8)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(require 2htdp/image)
...
(define (colorbar s l loc)
  (local [(define (n<l? color)
            (< (string-length color) l))
          (define (squ color)
            (square s "solid" color))]
    (foldr beside empty-image (map squ (filter n<l? loc)))))
