...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |mt2-p6-starter (5)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #t)))
(require spd/tags)
(require 2htdp/image)
...
(define (pyramid n)
  (cond [(= n 0) empty-image]
        [else
         (local [(define (single n)
                 (rectangle SIZE (* SIZE (+ n 1)) PEN COLOR))
                 (define (combine r rnr)
                   (beside/align "bottom"
                                 r
                                 rnr
                                 r))]
           (foldr combine empty-image (build-list n single)))]))
